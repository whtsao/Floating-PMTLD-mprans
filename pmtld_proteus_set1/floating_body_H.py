...
vertices = [(-2.*wavelength, 0.00),			# 0 domain boundary: 8 nodes
	    ...
	    (0.5*water_length+0.5*tld_l, water_level),	# 9
            ...
            (0.5*water_length-0.5*body_l, water_level-bottom_h), # 12 body: 8 nodes
            (0.5*water_length+0.5*body_l, water_level-bottom_h), # 13
            (0.5*water_length+0.5*body_l, water_level-bottom_h+body_h), # 14
            (0.5*water_length+0.5*body_l-tw, water_level-bottom_h+body_h), # 15
            (0.5*water_length+0.5*body_l-tw, water_level), # 16
            (0.5*water_length-0.5*body_l+tw, water_level), # 17
            (0.5*water_length-0.5*body_l+tw, water_level-bottom_h+body_h), # 18
            (0.5*water_length-0.5*body_l, water_level-bottom_h+body_h)] # 19
...
segments = [(i, (i+1)%nVertices) for i in range(nVertices)] + [(1,6)] + [(2,5)] + [(10,11)]
...
regions = [[-0.1, 0.1], # generation = 1
	   ...
	   [0.5*water_length, water_level-0.5*bottom_h]] # body = 5

regionFlags=np.array([1,2,3,4,5])
...
holes = np.array([0.5*water_length, water_level-0.5*bottom_h]) 

#regionFlags = np.array([1])
#boundaryTags = {'wall': 1}
# barycenter
barycenter = np.array([0.5*water_length, water_level, 0.])
